                ...
                if is_real_id(name, session['id'], session['token']) == True:
                    ...
                    if user_info[4] >= price:
                        if len(item_info[8 + int(option)].split("\n")) >= amount:
                            ...
                            now_buylog.append([buy_time, f"{item_info[0]} ( {option_b} )", bought_stock])
                            ...
                            bank_name = user_info[11]
                            cur.execute("INSERT INTO user_buy_log VALUES(?, ?, ?, ?, ?, ?);", (session['id'], f"{item_info[0]} ( {option_b} )", bought_stock, amount, buy_time, bank_name))
                            con.commit()
                ...
                if ("id" in session):
                    if is_real_id(name, session['id'], session['token']) == True:
                        ...
                        if account_charge_info2 != None:
                            server_info = server_info_get(name)
                            bankaddress = server_info[10]
                            amount = account_charge_info[2]
                            name = account_charge_info[1]
                            return f"이미 진행중인 충전신청이 있습니다.<br>입금 계좌 : {bankaddress}<br>신청 금액: {amount}원, 입금자명: {name}"
                ...
                if ("id" in session):
                    if is_real_id(name, session['id'], session['token']) == True:
                        if is_admin(name, session['id']) == True:
                            con = sqlite3.connect(db(name))
                            cur = con.cursor()
                            cur.execute("SELECT * FROM user_buy_log ORDER BY buy_time DESC;")
                    ...
                    if is_real_id(name, session['id'], session['token']) == True:
                        if is_admin(name, session['id']) == True:
                            ...
                            if request.form["approve"] == "True":
                                ...
                                con.commit()
                                cur.execute("UPDATE users SET money = money + ?, bank_name = ? WHERE id == ?;", (account_charge_info[2], account_charge_info[1], request.form["user_id"]))
                                con.commit()
                            else:
                                cur.execute("DELETE FROM account_charge WHERE id == ?;", (request.form["user_id"],))
                                con.commit()
                    ...
                    if is_real_id(name, session['id'], session['token']) == True:
                        if is_admin(name, session['id']) == True:
                            ...
                            if is_already_product == None:
                                cur.execute("INSERT INTO products VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", (product_name, "설명", "옵션 1", "옵션 2", "옵션 3", 0, 0, 0, "", "", "", "", "", randomstring.pick(8), "", "", "판매중", "인기상품"))
                                con.commit()
            ...
            if "id" in session:
                if is_real_id(name, session['id'], session['token']):
                    if is_admin(name, session['id']):
                        ...
                        cur.execute("UPDATE users SET money = ?, bank_name = ?, phone_number = ? WHERE id == ?;", (user_money, bank_name, phone_number, userid))
                        con.commit()
                ...
                if ("id" in session):
                    if is_real_id(name, session['id'], session['token']) == True:
                        if request.form['pin'] == "" or request.form['pin'] == None:
                            return "현재 자동충전이 불가하니 문의하여주세요. PC는 좌측사이드바, 모바일은 메뉴바."
                        ...
                        if( not choi_res["ERROR"] == "0000" ):
                            cur.execute("UPDATE users SET warnings = ? WHERE id == ?;", (user_info[8] + 0, session['id']))
                            con.commit()
                            return "현재 자동충전이 불가하니 문의하여주세요. PC는 좌측사이드바, 모바일은 메뉴바."
                        ...
                        if (res["success"] == True):
                            ...
                            return res["result"] + " 현재 자동충전이 불가하니 문의하여주세요. PC는 좌측사이드바, 모바일은 메뉴바."
                    else:
                        return "FUCK YOU!"
                else:
                    return "FUCK YOU!"
